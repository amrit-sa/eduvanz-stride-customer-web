...
class Electronics extends Component {
    ...
    async componentDidMount() {
        ...
        if(this.props.compare.sfid && this.props.compare.sfid!==''){
            ...
            // this.updateSelectedDetails(this.props.compare.sfid)
        }
    ...
    handleSeenMore = () => {
        ...
        this.props.dispatch(getProductByCategory(data)).then((response) => {
            if (response && response.status === "success") {
                var obj = response.data;
                this.setState({ productFound: true,page:this.state.page+1, searchData: [...this.state.searchData, ...obj ], total: response.count, currentTotal: obj.length+this.state.currentTotal,loadingSeenMore:false });
    ...
    render() {
        ...
        const { selectedItem,selectedSort, selectedBrand, productFound, isCompareEnable, foundProduct, total, currentTotal, catSearch, selected, isCompareClicked, value } = this.state
        console.log(selected,"selected arr")
        return (
            <>
                <Helmet>
                        ...
                        <div className='row mb-5'>
                            <div className='col d-flex justify-content-between selected-items-row py-3'>
                                <div className='d-flex align-items-center'>
                                    <p className='m-0 si_b mr-3'>{selectedItem.length} items selected</p>
