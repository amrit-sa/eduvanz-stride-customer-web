...
import $ from 'jquery';
import { favProduct, getProductByCategory } from "../actions/user";
...
class TrandingProduct extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.dispatch(getProductByCategory(data))
            .then((response) => {
                if (response && response.status === "success") {
                    for (let i = 0; i < response.data?.length; i++) {
                        if (response.data[i].trending__c == true) {
                        ...
                        <div className="container">
                            <div className="row">
                                <div className="col mb-2 laptops">
                                    <div className="d-flex justify-content-between align-items-center mb-2 shopby-after-bg-image position-rel"><h3 className="section_title mb-lg-4 mb-3">Trending Products</h3><a href="#" className="text-decoration-none fs-14 font-weight-bold d-block">View All <i className="ml-2 fa fa-angle-right fs-14 font-weight-bold" aria-hidden="true"></i></a></div>
                                    <Carousel responsive={responsive}>

                                        {this.state.lap_products.length > 0 ? (
